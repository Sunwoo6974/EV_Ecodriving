...
JTtemp1 = (Sf'-Sh);
JTtemp2 = 0*JTtemp1;
%JTtemp1(JTtemp1<0 | JTtemp1>100)=inf;
JTtemp2(JTtemp1<0 | JTtemp1>100)=inf;
JT = repmat(JTtemp2,1,1,size(Vh,2));
JT = permute(JT,[1 3 2]);
%JTadd = zeros(size(JT));
%{
...
for Stage = Totalstage-1:-1:1
    Tstart = tic;
    J{Stage,1} = SPiT_3state_back_Val2(Stage,Coef,T,T_qu,Sh,Sh_qu,Vh,Vh_qu,Sf,Sf_qu,Force_qu,Slope(Totalstage-Stage),J{Stage+1,1},Totalstage,Min_dg,Theta);
    TElapsed = toc(Tstart);
