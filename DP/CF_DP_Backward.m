...
fv_vtemp = table2array(fv_cycle(:,2))/2.237; % [km/h] -> [m/s]
fv_T = [0:0.1:fv_timetemp(end)]';
fv_V = interp1(fv_timetemp,fv_vtemp,fv_T,'linear');
fv_S = td2sd(fv_V)+init_gap; % 거리
...
for i1=Totalstage-1:-1:1
    tstart = tic;
    J{i1,1} = CF_DP_Back_Val(i1,coef,V,T,S,Vqu,Tqu,Squ,fv_V,fv_T,fv_S,slope(Totalstage-i1),J{i1+1,1});
    
    %{
    if (i1 ~= Totalstage || i1 ~= 1)
    J{i1,1}(:,:,1) == J{i1+1,1}(:,:,1)
    end
    %}
