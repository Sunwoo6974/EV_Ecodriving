function J_dg = dgpreview_f(hv_vnext,hv_tnext,hv_snext,fv_v,fv_t,fv_s,Tqu)
%dg_temp = zeros(size(hv_snext));

r = find(abs(hv_tnext*Tqu - fv_t) < 0.01);
...
%J_dgtemp1 = ~(dg_temp < safe_dg & dg_temp > safe_dg-50); % Backward Step #8~9 //
%J_dgtemp1 = ((dg_temp < 2) | (dg_temp > 100)); % Backward Step #8~9 //
J_dgtemp1 = (dg_temp < 20);% | (dg_temp > 50)); % Backward Step #8~9 //
